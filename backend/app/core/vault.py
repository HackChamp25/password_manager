...
class VaultManager:
    ...
    def _verify_master_password(self, fernet: Fernet) -> bool:
        ...
        if self.failed_attempts >= MAX_LOGIN_ATTEMPTS:
            if current_time - self.last_attempt_time < LOGIN_ATTEMPT_DELAY * (
                2 ** min(self.failed_attempts - MAX_LOGIN_ATTEMPTS, 5)
            ):
                return False
        try:
            if not os.path.exists(VERIFY_FILE):
    ...
    def save_data(self, data: Dict) -> None:
        ...
        try:
            ...
            vault_structure = {"data": data_json_str, "hmac": mac.hex()}
